...
I built a simple command-line tool last week for analyzing the token usage of the Gemini CLI. It has been open-sourced on GitHub. You can find it at [ceshine/gemini-cli-usage-analyzer](https://github.com/ceshine/gemini-cli-usage-analyzer). This is a blog post that briefly describes why I built this tool, the challenges I encountered during development, and how I dealt with them.

Note: Currently it only supports analyzing individual projects, unlike Claude Code, which analyzes the overall (cross-project) usage by default. This is because Gemini CLI does not provide a built-in mechanism to manage logs from different projects in a single place (in Claude's case, the `~/.claude` folder in Linux). Support for aggregating statistics across multiple projects is on the development roadmap.

{{< figure src="output-screenshot.png" caption="Sample Output of the Tool" >}}

## Motivation

I wanted to evaluate my token usage in the Gemini CLI to determine whether the paid subscription was worth it for me. Gemini CLI's free and subscription plans are [notoriously not transparent about their quotas](https://github.com/google-gemini/gemini-cli/issues/11772#issuecomment-3469732392). An analysis of tokens used by Gemini CLI based on my actual daily development work would be the best way to determine if the $20 or $249 monthly subscription fees would be more cost-effective than simply using paid API requests to power Gemini CLI.

I looked for a Gemini CLI equivalent of the [ccusage](https://github.com/ryoppippi/ccusage) tool for Claude Code, but I couldn't find one. Gemini CLI does not provide simple, centrally managed JSONL logs in a single folder, as Claude Code does. Fortunately, it does provide more generic [OpenTelemetry logs](https://geminicli.com/docs/cli/telemetry/#file-based-output-recommended), which can be consumed by OpenTelemetry-compatible tools. The bad news is that most OpenTelemetry-compatible tools I found are focused on enterprise-level production use cases, which is overkill for this personal usage analysis.

In the end, I decided to build a simple command-line tool similar to `ccusage` using Gemini CLI to achieve these three objectives:

1. Learn how to process the OpenTelemetry log files generated by Gemini CLI, which could be beneficial for performing deeper analysis in the future (e.g., to debug the agentic workflow).
...
Although we could split the JSON objects using the `\n}\n{\n` delimiter, that delimiter is not a standard format supported by common JSON-reading tools. Therefore, it is more appropriate to convert the file into the standard JSONL format for easier processing. Additionally, there are many log entry types that are not relevant to our analysis, and for the relevant entries, many fields can still be pruned without affecting the analysis results. We can perform pruning and simplification on the JSONL files to reduce the size of these files. This is an important feature because the raw OpenTelemetry file can easily grow to a few hundred megabytes after a couple dozen interactions with the Gemini CLI. 
...
To avoid data loss and other potential issues, we are not removing the raw OpenTelemetry log file by default. The user needs to specifically specify the `--enable-archiving` flag to do so. This creates another problem. We need a way to identify each log entry (a JSON object) to avoid ingesting it into the JSONL file multiple times. Unfortunately, I did not find any unique identifier in the log entry, so the solution I came up with is to use the `event.timestamp` values to seperate entries that have already been ingested from those haven't. This solution is designed under the assumption that the log entries are being appended to the log file in chronological order.
