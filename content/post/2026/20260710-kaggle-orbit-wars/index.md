...
title: "[Kaggle] Quick Recap of Orbit Wars"
description: "Reflections on competing in Kaggle's Orbit Wars, from building a Rust simulation engine to learning hard lessons about complexity and prioritization."
tags:
  - python
  - kaggle
  - simulation
...
After giving myself some time to recharge [late last year](https://blog.ceshine.net/post/state-of-ai-2025/), one of my goals has been to participate in at least one Kaggle competition to see how things have changed, both for me and for the community. I finally picked Orbit Wars, a simulation competition, because I had a blast competing in [Google Research Football](https://www.kaggle.com/competitions/google-football), the last simulation competition I participated in, back in 2020. Despite having to drop the competition in early June because of the aforementioned family matter, I still had a lot of fun (not as much fun as last time, though) and learned a lot from the experience. Now that the competition has ended, a lot of good write-ups have been published. I want to briefly document my experience and lessons learned from this competition for future reference. I hope some readers will find it helpful as well.
...
You can find the full official description of the game in the [kaggle-environments](https://github.com/Kaggle/kaggle-environments/blob/master/kaggle_environments/envs/orbit_wars/README.md) GitHub repository.
...
1. Build a Rust library that accurately and efficiently reproduces the game mechanics, so I can project the game state given a list of launched fleets and planet garrisons, and construct a table of potential fleet traffic between planets.
...
2. I managed to build a heuristic-based agent under the desired framework that performed quite well on the leaderboard before [the Producer agent](https://www.kaggle.com/code/slawekbiel/the-producer-agent/comments) was published, fluctuating around rank 100, well within the silver medal range.
...
4. The geometry involved in this game is quite complicated. I took the opportunity to use AI agents to help me review the core mathematics behind some of the key calculations, such as the distance from a point to a line segment and the intersection of a tunnel and a line. The process helped me refresh my memory on some of the key geometric concepts, such as [the normal form equation of a line](https://flexbooks.ck12.org/cbook/ck-12-cbse-maths-class-11/section/9.8/primary/lesson/normal-form-equation-of-a-line/). I've also posted AI-generated learning material on [GitHub Gist](https://gist.github.com/ceshine/6d5e47b6fd48fe72143dea1e0802daae).
...
4. The sweep collision calculation changed midway through the competition (and became much more complicated), which forced me to rewrite an entire section of the Rust library and rendered my original design for fleet traffic calculations less efficient.
5. There were some subtle bugs in the Rust library that took me a while to uncover and fix. The coding agent was not very helpful. On the bright side, my grasp of the Rust implementation became much better through the process.
6. I left many of the delta calculations in the Python portion of the agent as I improved the heuristics. This helped move things forward faster; however, it also made the code very buggy. I ended up spending a lot of time debugging and building a tool to visualize the agent's decisions.

### Lessons Learned

1. Although [the Producer agent](https://www.kaggle.com/code/slawekbiel/the-producer-agent/comments) is based on the same core idea as my final agent, it is implemented much more elegantly. It makes some simplifications (e.g., ignoring comets and using the same fleet size for all candidate fleets from a source planet) while putting significant effort into optimizing and accurately implementing parts of the game logic.
